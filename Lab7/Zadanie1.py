...
class ImageAligning(BaseImage):
    ...
    def align_image(self, tail_elimination: bool = True) -> 'BaseImage':
        ...
        if tail_elimination is True:
            """
            image = image.histogram().to_cumulated()
            max = 95% of pixels are below that value
            min = 5% of pixels are above that value
            loop through pixels:
            exposure = (exposure - min(pixel)) * 255/(max(pixel)-min(pixel))
            """
        #elif tail_elimination is False:
            """
            loop through pixels:
            pixel exposure = ( pixel exposure - min(pixel)) * 255/(max(pixel)-min(pixel))
            """
        #return image
