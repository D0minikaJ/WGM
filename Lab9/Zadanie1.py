...
class ImageFiltration:
    def conv_2d(self, image, kernel: np.ndarray, prefix: Optional[float] = None) -> BaseImage:
        ...
        kernel_size = np.kernel.shape(0)

        #row = np.image.shape(0)
        #column = np.image.shape(1)
        ...
        for row in image:
            ...
            for column in image:
                ...
                for a in range(kernel_size*(-1), kernel_size):
                    for b in range(kernel_size*(-1), kernel_size):
                        pixel = kernel[a, b] * image[row - a, column - b]
                new_row.append(pixel)
...
for row in image:
    ...
    for pixel in image:
        for a in range(kernel_size * (-1), kernel_size):
            for b in range(kernel_size * (-1), kernel_size):
                pixel = kernel[a, b] * image[row - a, pixel - b]
        new_row.append(pixel)
    new_image.append(new_row)
plt.imshow()
